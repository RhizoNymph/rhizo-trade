...
def calculate_returns(group, future_periods=[1, 3, 5, 7, 14, 30], past_periods=[1, 3, 5, 7, 14, 30], frequency='d'):
    ...
    # for period in past_periods[1:]:
    #     # Calculate VWMA directly
    #     group = group.with_columns(
    #         ((pl.col("close_price") * pl.col("dollar_volume")).rolling_sum(window_size=period) /
    #         pl.col("dollar_volume").rolling_sum(window_size=period)).alias(f"vwma_{period}{frequency}")
    #     )

    #     # Calculate normalized distance from VWMA
    #     group = group.with_columns(
    #         ((pl.col("close_price") - pl.col(f"vwma_{period}{frequency}")) /
    #         pl.col(f"vwma_{period}{frequency}")).alias(f"vwma_{period}{frequency}_dist")
    #     )
...
def calculate_return_stdev(group, past_periods=[1, 3, 5, 7, 14, 30], frequency='d'):
    """Calculates the standard deviation of past returns for a given group.

    Args:
        group (polars.DataFrame): The input DataFrame. Must have return columns generated by `calculate_returns`.
        past_periods (list): List of periods for which to calculate standard deviation.
        frequency (str): Frequency of the data ('d' for daily, 'h' for hourly, etc.).

    Returns:
        polars.DataFrame: DataFrame with added standard deviation columns.
    """
    for period in past_periods:
        return_col_name = f'return_{period}{frequency}'
        if return_col_name in group.columns:  # Check if the return column exists
            group = group.with_columns(
                pl.col(return_col_name).rolling_std(period).alias(f'return_std_{period}{frequency}')
            )
        else:
            print(f"Warning: Column {return_col_name} not found. Skipping standard deviation calculation for this period.")
    return group
